defmodule TireDispatchWeb.UserAuthTest do
  use TireDispatchWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias TireDispatch.Users
  alias TireDispatch.Users.Scope
  alias TireDispatchWeb.UserAuth

  import TireDispatch.UsersFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, TireDispatchWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      TireDispatchWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: TireDispatchWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        TireDispatchWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
